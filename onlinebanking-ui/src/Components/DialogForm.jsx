...
export default function DialogForm(props){
    ...
    const handleSubmit = async (event) => {
        ...
        if (props.action === "Create") {
            ...
            try{
                const response = await axios.post(`http://localhost:8080/addAccount/${userData.userId}`,formData)
    ...
    return (
        ...
        <Dialog open={props.open} onClose={handleClose}>
            ...
            <FormControl style={{margin:"10px"}} >
                ...
                <br />
                <TextField name="balance" onChange={handleChange}  label="Balance" value={formData.balance}>
                </TextField>
                <br />
        ...
        <Dialog open={props.open} onClose={handleClose} >
            ...
            <div style={{padding:"20px"}}>
                ...
                {props.accounts.map((account)=>
                    <Account key={account.accNo} account = {account} handle={handleDelete} action="Delete"/>
                )}
            </div>  
        </Dialog>
        }
        ...
        <Dialog open={props.open} onClose={handleClose} >
            ...
            <div style={{padding:"20px"}}>
                ...
                <i>(Type - Account Number - Balance)</i>
                {props.accounts.map((account)=>
                    <Account key={account.accNo} account = {account} handle={handleDisable} action="Disable"/>
                )}
            </div>  
        </Dialog>}
        ...
        <Dialog open={props.open} onClose={handleClose} >
            ...
            <div style={{display:"flex", flexDirection:"column"}}>
                <div style={{padding:"20px"}}>
                    ...
                    <i>(Type - Account Number - Balance)</i>
                    {props.accounts.map((account)=>
                        <Account key={account.accNo} account = {account} handle={handleDisable} action="View"
                ...
                <div style={{border:"1px solid black", margin: "20px"}}>
                        <div>
                        <p style={{textDecoration:"underline"}}><i><strong>History</strong></i></p>
                        <table>
                            <thead>
                            <tr>
                                <th> TimeStamp </th>
